...
public class dispatcher4 {
	Logger logger = LoggerFactory.getLogger(dispatcher4.class);
	...
	private static ArrayList<URL> visitedURLs = new ArrayList<URL>(); // �������ڴ����ݿ�
																		// ����ȥ��
	public static ArrayList<URL> rangeURLs = new ArrayList<URL>(); // ��ʱList
																	// �������ڴ����ݿ�
	public static ArrayList<URL> IsrangeURLs = new ArrayList<URL>(); // ��ʱlist
																		// �������ڴ����ݿ�
	...
	public synchronized String analyzergetURL(String analyzer) throws Exception // ����gather��ţ��������ܵ���
	{
		...
		logger.info("�̡߳�analyzer��" + analyzer + "��ȡURL:" + url3);
		filter.add(url3.toString());
		visitedURLs.add(url3);// ���뵽�Ѿ����ʵ�vistedURL
		logger.info("--------------------------------------�Ѿ�ץȡ��URL:" + visitedURLs.size());
		// return conurl;
...

	{
		...
		if (!filter.contains(newURL)) // &&!rangeURLs.contains(url)
		{
			...
			logger.info("-------------fliter�ж��ظ���URL:" + newURL);
		}
	...
	public synchronized void insert1(ArrayList<String> analyzedURL) {
		...
		for (String url : analyzedURL) {
			...
			// url.getHost()
			logger.info("---�ж�2��URL�Ƿ�ͨ��:" + memoryQueue.peek() + "=======���д�С" + memoryQueue.size());
			logger.info("-------------�ȴ��������еĴ�С" + memoryQueue2.size());
		}
	}
	...
	public ArrayList<URL> range(ArrayList<URL> analyzedURL) // �ж������Ƿ��ڷ�Χ֮��**********
	{
		...
		for (URL url : analyzedURL) {
			logger.info("---�ж�3��URL:" + url + "=====����:" + url.getHost());
			if (rangeURLs.contains(url))// &&!rangeURLs.contains(url)
				IsrangeURLs.add(url);
			logger.info("---�ж�3��ͨ����URL����:" + IsrangeURLs.size() + "=======���д�С" + memoryQueue.size());
